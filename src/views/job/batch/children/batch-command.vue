<template>
  <el-card>
    ...
    <div class="card-content">
      <h1>批量命令</h1>
      <el-form :model="form" label-width="auto" style="max-width: 100%; margin-top: 15px;">
        <el-form-item label="执行类型" prop="batchType" style="width: 100%;" required>
          <el-select
        ...
        <el-form-item label="执行对象" prop="serverList" style="width: 100%;" required>
          <el-select
          ...
          >
            <el-option
              ...
              :value="item.ID"
            />
            <el-option v-if="!batchObjOption.length" :value="-1" disabled label="加载中..."></el-option>
          </el-select>
        </el-form-item>

        <el-form-item label="命令" prop="commandId" style="width: 100%;" required>
          <el-select
          ...
          >
            <el-option
              ...
              :value="item.ID"
            />
            <el-option v-if="!commandList.length" :value="-1" disabled label="加载中..."></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      ...
      >
        <template #default>
          <p>执行类型: <strong>{{ getBatchTypeLabel }}</strong></p>
          <p>执行对象: <strong>{{ batchObjName }}</strong></p>
          <p>执行命令: <strong>{{ commandInfo }}</strong></p>
        </template>
        <template #footer>
...
const commandInfo = computed(() => {
  return commandList.value.find(item => item.ID === form.value.commandId)?.command || ""
})

const batchObjName = computed(() => {

  let objList = batchObjOption.value.filter(item => form.value.serverList.includes(item.ID))

  switch (form.value.batchType.toString()) {
    case "1":
      return objList.map(item => item.serverName).join(",")
    case "2":
      return objList.map(item => item.platformName).join(",")
    default:
      return "1111"
  }
})
...
const getBatchTypeLabel = computed(() => {
  ...
  const option = batchTypeOption.find(option => option.value === batchTypeNumber);
