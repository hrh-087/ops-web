...
const init = async () => {
  ...
  await CasbinAPI.getPolicyPathByAuthorityId({authorityId:activeUserId.value}).then((res) => {
    const arr:never[] = []
...
const authApiEnter = () => {
  ...
  CasbinAPI.updateCasbin({authorityId:activeUserId.value, casbinInfos}).then((res) => {
    if (res.code === 0){
