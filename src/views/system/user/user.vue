...
// 获取用户信息
const getTableData = () => {
  UserAPI.getUserList({page:page.value, pageSize:pageSize.value}).then((res)=>{
...
// 初始化相关
const setAuthorityOptions = (AuthorityData:never[], optionsData:Array<any>) => {
...
const switchEnable = async(row:any) => {
  ...
  if (res.code === 0) {
    ElMessage({ type: 'success', message: `${!req.enable ? '禁用' : '启用'}成功` })
    await getTableData()
    // userInfo.value.headerImg = ''
    userInfo.value.authorityIds = []
  }
...
const resetPasswordFunc = (row:any) => {
  ElMessageBox.confirm(
    ...
    UserAPI.resetPassword({ID: row.ID,}).then((res:any) => {
      ElMessage({
        type: 'success',
        message: res.msg,
      })
...
const setAuthorityIds = () => {
  tableData.value && tableData.value.forEach((user:any) => {
    user.authorityIds = user.authorities && user.authorities.map((i:any) => {
