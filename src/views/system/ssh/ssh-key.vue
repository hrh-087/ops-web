<template>
  <div class="ops-table-box">
    <div class="ops-btn-list">
      <el-button type="primary" icon="plus" @click="addSshKey()">
        新增秘钥
      </el-button>
    </div>
    <el-table
  ...
  >
    ...
    <el-form ref="sshKeyForm" :model="form" :rules="rules" label-width="150px">
      ...
      <el-form-item label="私钥密码" prop="privateKeyPassphrase">
        <el-input v-model="form.privateKeyPassphrase" autocomplete="off" />
      </el-form-item>

      <el-form-item label="是否使用密码登录" prop="usePass">
...
const enterDialog = () => {
  ...
  sshKeyForm.value?.validate((valid: boolean) => {
    if (valid) {
      switch (type.value) {
        case "add": {
          SshKeyApi.createSshKey(form.value).then((res: any) => {
            ElMessage({
              type: "success",
              message: "添加成功!",
            });
        ...
        case "edit": {
          SshKeyApi.updateSshKey(form.value).then((res: any) => {
            ElMessage({
              type: "success",
              message: "修改成功!",
            });
